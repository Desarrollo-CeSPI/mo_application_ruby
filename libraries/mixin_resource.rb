    ...
    def initialize(name, run_context=nil)
      ...
      @before_migrate = Proc.new do
        ...
        rbenv_execute "Run bundle install #{me.name}" do
          command "bundle install --binstubs #{bundle_binstubs} --path #{bundle_path} --without #{Array(me.bundle_without_groups).join} --jobs #{me.bundle_install_jobs ? me.bundle_install_jobs : [4,[1,Integer(node.cpu.total*0.8)].max].min} && ls /jsahkjash"
          cwd release_path
